...
async def Work_with_Message(m: types.Message):
    ...
    if e.demojize(m.text) == "Продлить :money_bag:":
        ...
        if not payment_info is None:
            urltopay = CONFIG["url_redirect_to_pay"] + str((await p2p.check(bill_id=payment_info['bill_id'])).pay_url)[
                                                       -36:]
            ...
            await bot.send_message(m.chat.id,
                                   "<b>Оплатить можно с помощью Банковской карты или Qiwi кошелька!</b>\n\nВыберите на сколько месяцев хотите приобрести подписку:",
...
async def Buy_month(call: types.CallbackQuery):
    ...
    if payment_info is None:
        ...
        new_bill = await p2p.bill(amount=Month_count * CONFIG['one_month_cost'], lifetime=45,
                                  theme_code=CONFIG['qiwi_theme_code'],
...
async def AddTimeToUser(tgid, timetoadd):
    ...
    if int(userdat.subscription) < int(time.time()):
        ...
        await bot.send_message(userdat.tgid, e.emojize(
            'Данны для входа были обновлены, скачайте новый файл авторизации через раздел "Как подключить :gear:"'))
    else:
        ...
        try:
            ...
            if len(log) > 0:
                ...
                for i in log:
                    ...
                    if status == "EXPIRED":
                        BotChecking = TeleBot(BOTAPIKEY)
                        BotChecking.edit_message_text(chat_id=i['tgid'], message_id=i['mesid'],
                                                      text="Платеж просрочен.",
                                                      reply_markup=None)
                        db = sqlite3.connect(DBCONNECT)
                        db.execute(f"DELETE FROM payments where tgid=?",
                                   (i['tgid'],))
